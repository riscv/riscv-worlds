...
Smwid extension.  `Ceil(Log~2~NWorlds)` LSBs are used, others are zero.
...
determine its mutability. The lock bit is at offset `XLEN-1` within the CSR, and
...
.Smlwid extension: Support for M-mode to control the lower privilege modes' world
...
the `pmlwidlist` mask). Like `pmlwidlist`, `mlwidlist` (width of NWorlds)
represents a set of WIDs as a bit vector, with WID _i_ represented by bit _i_
of the register.
...
.Smlwid with Smlwidlist extension
...
.Smwdeleg/Sswid extensions: Support for S-mode to control the lower privilege mode's world.
