...
.One world per hart
image::images/drawio/one_world.svg[width=200]
...
modes can be provided. `mlwidlist` (width of NWorlds) represents a set
of WIDs as a bit vector, with WID _i_ represented by bit _i_ of the register.

If an unauthorized WID (per the `mlwidlist`) is written to `mlwid`, when the
...
Similarly, after a MPRIV update in `mstatus` to modify the effective privilege
mode to a less-privileged mode at which load and stores are executed, if the 
lower-privilege attempts to perform a load or store in an illegal world, the
hart will take a precise exception indicating the software-check cause.
...
image::images/drawio/Smwid.svg[width=400]
...
`mlwidlist`). It is up to the platform architect to decide whether to
allow this or not.
...
image::images/drawio/Smwdeleg_Sswid.svg[width=600]
