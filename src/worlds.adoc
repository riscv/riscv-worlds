...
transaction. The method of propagating the WID through caches is
platform-specific (addition of the WID to the tag is one possibility).
If a subsequent request matches on the physical address, the WID of the
original transaction must be considered to determine whether it is
a cache hit or a cache miss (with eviction). The cache policy is again
platform-specific.
...
The WID of M-mode on a hart (`mwid`) is set by the external environment and
does not change between resets.
...
NOTE: The `mwid` can be set via a pinstrap, a fuse or a secure SoC register.
...
search for valid WIDs. Like any other WARL register, software is expected
to read back the configured value to verify it is indeed legal.
