...
* The *Smlwidlist* extension depends on the *Smwid* extension and introduces
  the `mlwidlist` CSR to allow M-mode to restrict the set of WIDs that may be
  used with `mlwid`. The `mlwidlist` CSR is locked (read-only) whenever `mwid`
  is locked.
* Next comes the *Smwdeleg* extension, which further enables
  M-mode to delegate to lower-privilege modes the ability to assign the WID of
  next lower-privilege modes. Without the H extension, this means S-mode may set
  the WID to be used by U-mode. This is defined by the *Sswid* extension to S-mode
  with the `slwid` CSR. With the H extension, this means HS-mode may set the WID for
  VS-mode and lower.
* The *Shlwid/Shwdeleg* extensions add another delegation level to match the additional
  ...
  *Shwdeleg* also adds a VS replica CSR called `vslwid` which is accessed from HS-mode
...
The RISC-V Worlds Smlwid, Smwdeleg, Sswid, Shwdeleg and Shlwid extensions
allow a hart to assign WIDs to its privilege modes and masks to control the
values that are authorized for use with the principle that no privilege mode
can change its own WID. The CSRs for these extensions are listed in the Table
below.
...
values delegated to (H)S-mode, represented as a bit vector. `NWorlds` LSBs are used,
others are zero.
| `XLEN` | `slwid` | WARL for S, HS | `0x190` | WID value used in lower privilege
modes than S-mode (i.e., U-mode). `Ceil(Log~2~NWorlds)` LSBs are used,
others are zero.
| `XLEN` | `hwiddeleg` | WARL for HS | `???` | Set of WID
values delegated to VS-mode, represented as
a bit vector. `NWorlds` LSBs are used, others are zero.
| `XLEN` | `hlwid` | WARL for HS | `???` | WID value used for lower privilege
modes than HS-mode (i.e., VS/VU/U-mode).
 `Ceil(Log~2~NWorlds)` LSBs are used, others are zero.
| `XLEN` | `vslwid` | WARL for HS | `???` | this CSR is the VS replica register of
`slwid`. In HS-mode this register can be programmed to set the effective WID of
VU mode. In VS-mode accesses to `slwid` use the `vslwid` contents.
`Ceil(Log~2~NWorlds)` LSBs are used, others are zero.
|===
...
(Shwiddeleg, Shlwid, Sswid) are disabled. So any accesses to the `slwid` CSR
raises an illegal instruction exception as well as accesses to `hwiddeleg`
and `hlwid` (if supported).
Otherwise, if `mwiddeleg` is non-zero, the Sswid extension is enabled and optional
Shwiddeleg/Shlwid may be enabled. Like any other WARL register, M-mode software
is expected to read back the register to determine the actual configuration
...
At reset, `mwiddeleg` is set to zero and hence Sswid (and Shwdeleg/Shlwid)
are disabled.

=== Shlwid / Shwdeleg extensions
...
Similarly, any attempt by HS-mode or U-mode to perform a hypervisor load or store
"as VS/VU-mode" will be subject to the same `hwiddeleg` authorization check, meaning
...
.Shwdeleg/Shlwid extensions: Support for HS-mode to control the lower privilege mode's world.
image::drawio/Shwdeleg_Shlwid.svg[Shwdeleg_Shlwid,align="center"]

The Shwdeleg extension requires the Smwdeleg/Shlwid/Sswid extensions and allows
...
the `vslwid` replica CSR is defined as part of the Shwdeleg extension (see
   <<hypervisor.adoc#_hypervisor_and_virtual_supervisor_csrs, VS CSRs in the Hypervisor
   chapter>>).
In HS-mode this register can be programmed to set the effective WID of VU mode.
In VS-mode it takes over all functions of the `slwid` CSR.

The Shwdeleg extension adds the `hwiddeleg` HS-mode read-write CSR. The
...
any accesses to the `vslwid` CSR (direct from HS-mode, or indirect from the VS-mode
`slwid` replica) raise an illegal instruction exception.
Otherwise, if `hwiddeleg` is non-zero, the `vslwid` CSR is enabled. Like
any other WARL register, HS-mode software is expected to read back the register
to determine the actual configuration retained by the implementation.

At reset, `hwiddeleg` is set to zero and hence `vslwid` is disabled.
