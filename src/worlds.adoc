...
NOTE: M-mode software can not discover which worlds are available to be assigned
to lower modes nor which resources are accessible in each world.  The platform
software will have predetermined allocations for the worlds on a platform and 
all required information needed by M-mode software must be provided out-of-band.
...
sets the WID used for modes lower than [H]S-mode.  The `slwid` field is WARL
...
verify it is indeed legal. An unauthorized WID (per the `mwiddeleg` list) can
...
exception. `mlwid` world is assigned to [H]S-mode and lower privilege modes.
