...
As non-zero values are reserved, SW cannot rely on their behavior, so expectation
is that implementations make them read-only zero. The register is formatted as
