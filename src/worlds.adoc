...
* With the *Smwid* extension, the `mwid` CSR is introduced to provide a
  lockable control of the WID value assigned to the hart.
...
* The *Smlwidlist* extension depends on the *Smwid* extension and introduces
  ...
  is locked.
* Next comes the *Smwdeleg* and *Smwdelegh* extensions, which further enable
...
The RISC-V Worlds Smwid, Smlwid, Smlwidlist, Smwdeleg, and Sswid extensions
allow a hart to assign WIDs to its privilege modes and masks to control the
values that are authorized for use.  The new CSRs are listed in the Table
below.
...
| `XLEN` | `mwid` | WARL for M | ??? | WID used for M-mode.
...
M-mode by the platform, represented as a bit vector.  `NWorlds` LSBs are used,
others are zero.
...
=== Worlds-capable with no extensions enabled
...
platform-defined, we refer to it as `pwid`, and it might take any non-ISA form
...
determine its mutability. The lock bit is at offset 31 within the CSR, and
observes W1SS semantics. The WID field occupies the least-significant bits of
the CSR.

On-reset, the `mwid` CSR is expected to show the platform-defined `pwid` value.
...
non-ISA `pwidlist` mask - assigning a WID value to `mwid` that is not enabled
by that mask will result in the hart's transactions being trapped. This will
...
.Smwid extension: shown here with an unlocked-on-reset `mwid`.
...
least two privilege modes. Note: Smlwid is not dependent on Smwid.
...
on this hart. (This may be constrained by a non-ISA `plwidlist` mask.)
...
enabled in the platform's `plwidlist` mask, then the lower-privilege mode's
first attempt to fetch an instruction will take a precise exception.  This
exception, _like all exceptions generated by unauthorized WIDs_, will indicate
the "software-check" cause.  Similarly, after an MPRV update in `mstatus` to
...
fault (code=4)".  The software-check exception caused by Smwid has higher
...
.Smlwid extension: Support for M-mode to control the lower privilege modes' world, shown here without Smwid.
...
same world as M-mode. (`pwid` must be included in `pwidlist` but it is not
necessarily included in `plwidlist`.) It is up to the platform to determine
whether or not to allow this.

=== Smlwidlist extension

The *Smlwidlist* extension depends on the *Smwid* extension and introduces the
`mlwidlist` CSR to allow M-mode to further restrict the set of WIDs that may be
used with `mlwid` (beyond any limitations already imposed by the platform via
the `plwidlist` mask). `mlwidlist` (width of NWorlds) represents a set of WIDs
as a bit vector, with WID _i_ represented by bit _i_ of the register.

The `mlwidlist` CSR is locked (read-only) whenever `mwid` is locked, it does
not have a distinct lock bit.

.Smlwidlist extension: shown here with an unlocked-on-reset `mwid`.
...
that is not enabled in the platform's `plwidlist` will be overriden. I.e. a
...
for S-mode. Note: these extensions are not dependent on Smwid nor Smlwidlist.
...
`plwidlist` (if implemented by the platform). Attempts to authorize a WID in
`mwiddeleg` that is not authorized in `mlwidlist`/`plwidlist` will fail - the
WARL behavior will mask off any such bits. Simiarly, if `mlwidlist` is updated,
...
.Smwdeleg/Sswid extensions: Support for S-mode to control the lower privilege mode's world, shown here with Smwid, Smlwidlist, and a locked-on-reset `mwid`.
