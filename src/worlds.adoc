...
example, if M-mode uses MPRV to issue a transaction as S or U mode, it is
possible for that transaction to be eligible to generate an access fault (due
...
the `plwidlist` mask). The `mlwidlist` CSR is locked (read-only) whenever
`mwid` is locked.
...
.Smwdeleg/Sswid extensions: Support for S-mode to control the lower privilege mode's world, shown here with Smwid, Smlwidlist, and an unlocked-on-reset `mwid`.
image::drawio/Smwdeleg_Sswid.svg[]
