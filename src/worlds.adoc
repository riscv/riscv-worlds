...
* The *Shwdeleg/Svswid* extensions add another delegation level to match the additional
  ...
  via the `slwid` CSR addres in VS-mode.
...
The RISC-V Worlds Smlwid, Smwdeleg, Sswid, Shwdeleg and Svswid extensions
...
values delegated to S-mode, represented as a bit vector. `NWorlds` LSBs are used,
others are zero.
| `XLEN` | `slwid` | WARL for S, HS | `0x190` | WID value used in lower privilege
modes than S-mode (i.e., (V)U-mode). `Ceil(Log~2~NWorlds)` LSBs are used,
others are zero.
| `XLEN` | `hwiddeleg` | WARL for HS | `???` | Set of WID
values delegated to VS-mode with the optional Shwdeleg extension, represented as
...
| `XLEN` | `vslwid` | WARL for HS | `???` | With the Svswid extension, this CSR
 ...
 register can be programmed to set the effective WID of VU mode. In VS-mode
 accesses to `slwid` use the `vslwid` contents `Ceil(Log~2~NWorlds)` LSBs are
 used, others are zero.
|===
...
the first attempt to fetch an instruction by (V)U-mode will take a precise
...
=== Shwdeleg / Svswid extensions

The Shwdeleg extension requires the Smwdeleg/Sswid extensions and
allows HS-mode to delegate to VS-mode the ability to allocate WIDs to
VU-mode.
...
any accesses to the `hlwid` CSR raise an illegal instruction exception.
Otherwise, if `hwiddeleg` is non-zero, the Svswid extension is enabled.  Like
...
When this exception is raised, the `mtval` register is set to "illegal world
fault (code=4)". The software-check exception caused by Svswid has higher
priority than any access-fault exception (See <<exception-priority>> of 
Privileged Specification) and leads to a trap being delivered to M-mode.

NOTE: TBD - should this trap to HS-mode instead of M mode?

.Shwdeleg/Svswid extensions: Support for HS-mode to control the lower privilege mode's world.
image::drawio/Shwdeleg_Svswid.svg[Shwdeleg_Svswid,align="center"]
...
At reset, `hwiddeleg` is set to zero and hence Svswid is disabled.
