...
* The *Smwdeleg* extension, which further enables M-mode to delegate to S-mode
...
| `XLEN` | `mlwid` | WARL for M | `0x390` | WID used for lower-than-M privilege
levels. `Ceil(Log~2~NWorlds)` LSBs are used, others are zero.
...
| `XLEN` | `slwid` | WARL for S | `0x190` | WID value used in lower-than-S
...
lock-bit to determine its mutability. The lock bit (`L`) is at offset `XLEN-1`
within the CSR. The WID field occupies the least-significant
Ceil(Log~2~NWorlds) bits of the CSR.
