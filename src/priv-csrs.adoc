...
CSRs according to privilege level as shown in <<csrrwpriv>>. The top two
bits (csr[11:10]) indicate whether the register is read/write (`00`,`01`
...
|`hwiddeleg`  +
 `hlwid`
|Hypervisor WID values delegated to VS mode Register. +
 Hypervisor WID value for lower privilege modes Register. 
...
4+^|Virtual Supervisor Worlds Register 
|`???`
|HRW
|`vslwid`
|Virtual supervisor WID value for lower privilege modes Register.
...
4+^|Machine Worlds Registers
|`0x390`
|MRW
|`mlwid`
|Machine WID value for lower privilege modes Register.
...
`mwid` +
`mlwidlist`
| Machine WID values delegated to (H)S mode Register. +
Machine WID value for M-privilege mode Register. +
Machine WID values delegated to M mode Register.
